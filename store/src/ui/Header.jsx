...
const Navbar = () => {
  ...
  // Load selected counter from localStorage (for display only, not delivery address)
  const [selectedCounter, setSelectedCounter] = useState(null);
  
  useEffect(() => {
    const loadSelectedCounter = () => {
      try {
        const counterData = localStorage.getItem("selectedCounter");
        if (counterData) {
          const counter = JSON.parse(counterData);
          setSelectedCounter(counter);
          // DO NOT update deliveryLocation - that's for user's address only
        }
      } catch (error) {
  ...
  const fetchUserDeliveryAddress = useCallback(async () => {
    try {
      ...
      // Priority 1: Try to get default address from addressService (v1 API)
      try {
        const result = await addressService.getAddresses();
        if (result.success && result.data && result.data.length > 0) {
          // Find default address
          const defaultAddr = result.data.find(addr => addr.is_default);
          const addressToUse = defaultAddr || result.data[0]; // Use first if no default
          
          if (addressToUse) {
            setDeliveryLocation({
              pincode: addressToUse.postal_code,
              city: addressToUse.city
            });
            console.log("✅ Loaded delivery address from saved addresses");
            return; // Exit early if we found an address
          }
        }
      } catch (addressError) {
        console.log("No saved addresses found, checking last order...");
      }
      
      // Priority 2: If no saved address, try last order
      // Get token from localStorage (old method) or csrfToken (new method)
      ...
      // Add authentication (support both old and new methods)
      if (token) {
      ...
      // Get all orders for the user with authentication
      const response = await fetch(`${BASE_URL}/order/get-orders`, {
        method: "GET",
        headers: headers,
        credentials: "include" // Include cookies for new security method
      });
      ...
      if (userOrders.length > 0) {
        ...
        if (latestOrder.pincode && latestOrder.address) {
          ...
          console.log("✅ Loaded delivery address from last order");
        }
      }
    } catch (error) {
  ...
  const fetchUserMembership = useCallback(async (userId) => {
    try {
      // Get token from localStorage (support both old and new methods)
      const token = localStorage.getItem("token") || 
      ...
      const response = await fetch(`${BASE_URL}/v1/membership/subscription/${userId}`, {
        method: "GET",
        headers: headers,
      ...
      // Check if response is ok
      if (!response.ok) {
        console.error(`Membership API error: ${response.status} ${response.statusText}`);
      ...
      try {
        ...
        if (userData && userData.id) {
          ...
          if (userData.user_type === "Farmer") {
            fetchUserMembership(userData.id);
          }
        } else {
  ...
  }, [fetchUserDeliveryAddress, fetchUserMembership]);
      ...
      <div className="block md:hidden bg-white border-b shadow-md z-40">
          ...
          <div className="flex items-center space-x-2 flex-shrink-0">
            ...
            <div>
              {isLoggedIn ? (
                ...
                <button onClick={() => navigate("/login")} className="flex items-center text-sm font-medium text-gray-600" title="Login with Mobile">
        ...
        <div className="flex items-center justify-between px-2 py-1">
          ...
          <button 
            ...
            className="border border-blue-600 px-2 py-1 rounded text-blue-600 hover:bg-blue-50 transition-colors"
          >
            {t('header.change')}
          </button>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center lg:flex-row lg:justify-between lg:items-center px-3 py-1">
          ...
          <div className="flex justify-between items-center w-full lg:w-auto">
            ...
            <Link to="/" className="flex flex-col items-center">
              ...
              {userMembership && user?.user_type === "Farmer" && (
                <div className="flex items-center gap-1 bg-gradient-to-r from-yellow-400 to-yellow-600 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg mt-1">
                  <span>🌟</span>
                  <span>{userMembership.plan_id?.name || 'Member'}</span>
                </div>
              )}
            </Link>
          ...
          <div className="flex flex-col items-center lg:mt-4 w-full lg:w-auto">
            <div className="flex items-center mr-2">
              ...
              <div className="flex flex-col">
                <span className="text-md font-medium">{t('header.delivering')}  <span className="text-md text-orange-500 font-medium">{deliveryLocation.pincode || "Select Location"}</span></span>
                <span className="text-md flex items-center justify-center font-medium"><span className="text-md flex items-center justify-center text-orange-500 font-medium">{deliveryLocation.city || "No counter selected"}</span></span>
              </div>
              <FiEdit2
                className="ml-1 w-5 h-5 cursor-pointer text-blue-600 hover:text-blue-800 transition-colors"
                onClick={() => {
                  setManualMode(false);
