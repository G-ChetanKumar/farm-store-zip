...
const IntegratedPestManagement = () => {
  ...
  // Fetch and filter products by the "IntegratedPestManagement" category
  useEffect(() => {
    const fetchSeedProducts = async () => {
      try {
        ...
        // Find the "IntegratedPestManagement" category
        const IntegratedPestManagementCategory = categories.find(
          (category) => category.title === "Integrated Pest Management"
        );

        if (IntegratedPestManagementCategory) {
          const categoryId = IntegratedPestManagementCategory._id;
        ...
        console.error(
          "Error fetching Integrated Pest Management products:",
          error
        );
        setError(true);
      } finally {
            ...
            <div className="ml-4 flex-1">
              <h3 className="text-lg font-bold text-gray-800">
                ...
                <div className="flex items-center text-xs text-gray-600 mb-1 space-x-1">
                      <strong className="flex-shrink-0">MFG/MKTD BY :</strong>
                      <span className="text-red-400 truncate flex-1">{product.mfg_by}</span>
                      </div>
            </div>
          </div>
      ...
      <div className="px-2 sm:px-4">
        <div className="flex justify-between items-center mb-1">
          <div className="flex-1">
            <h2
              className="text-lg sm:text-2xl font-bold text-gray-800 leading-tight whitespace-pre-wrap break-words"
              style={{ maxWidth: "70%" }} // Optional to restrict heading's width
            >
