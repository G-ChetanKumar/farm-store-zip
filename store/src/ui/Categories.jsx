...
//   const handleRetry = () => {
//     setError(false); // Reset error state
//     setLoading(true); // Retry fetching categories
...
//   return (
//     <div className="w-full mx-1 border border-gray-300 box-border rounded-md relative bg-white">
...
//         </div>
//       ) : (
//         <div className="grid grid-cols-3 md:grid-cols-8 gap-3 p-2">
...
//             >
//               <div className="w-28 h-28 bg-gray-200 overflow-hidden border border-gray-300 rounded-md">
//                 <img
//                   src={item.imageUrl}
//                   alt={item.title}
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Reorder categories to place "Combo Offers" first
    ...
    <div className="w-full border border-gray-300 box-border rounded-md relative bg-white">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-3 md:grid-cols-8 gap-3 p-1">
          {displayedCategories.map((item) => (
            <Link
              to={`/category/${item.category_id}/subcategory/${item._id}/products?brandName=${encodeURIComponent(item.title)}`}
              key={item._id}
              className="flex flex-col items-center text-center"
            >
