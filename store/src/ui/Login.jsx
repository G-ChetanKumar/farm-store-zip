...
const Login = ({ setLogin }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // ✅ CRITICAL: Store CSRF token (new secure method)
      if (csrfToken) {
        localStorage.setItem("csrfToken", csrfToken);
        console.log("✅ CSRF token stored");
      } else {
        console.warn("⚠️ No CSRF token received from backend!");
      }
      
      localStorage.setItem("user", JSON.stringify(user));

      // Store the user_type separately for easier access
      if (user && user.user_type) {
        localStorage.setItem("userType", user.user_type);
      }

      console.log("✅ All tokens stored in localStorage");
      ...
      // ✅ FIX: Use navigate without reload to prevent race conditions
      // The axios interceptor will automatically use tokens from localStorage
      setTimeout(() => {
        navigate("/");
