...
import leftChevronIcon from "../assets/images/leftArrows.png";
import rightChevronIcon from "../assets/images/rightArrows.png";
...
const ProductsListDetail = () => {
  // Set default viewMode to 'list' on mobile, 'grid' otherwise
  ...
  const getFilteredSortedProducts = () => {
    ...
    // Sorting logic (same as handleSortChange)
    if (sortOption === "az") {
    ...
    <>
      ...
      <section className="py-1 bg-gray-100">
        <div className="px-2 sm:px-4">
          ...
          {/* Mobile: Sort/Filter and View Toggle Row */}
          <div className="flex justify-between items-center mt-4 md:hidden">
            {/* Left: Sort By and Filter By */}
          ...
          {showSortModal && (
            <div
              className="fixed inset-0 z-50 bg-black bg-opacity-40 flex items-end sm:items-center justify-center md:hidden"
              onClick={() => setShowSortModal(false)}
            >
              <div
                className="bg-white w-full sm:w-80 rounded-t-lg sm:rounded-lg p-4"
                onClick={e => e.stopPropagation()}
              >
                <h3 className="text-lg font-semibold mb-2">Sort By</h3>
                <ul>
                  <li>
                    <button className="w-full text-left py-2" onClick={() => { handleSortChange({ target: { value: 'az' } }); setShowSortModal(false); }}>Alphabetical (A-Z)</button>
                  </li>
                  <li>
                    <button className="w-full text-left py-2" onClick={() => { handleSortChange({ target: { value: 'za' } }); setShowSortModal(false); }}>Alphabetical (Z-A)</button>
                  </li>
                  <li>
                    <button className="w-full text-left py-2" onClick={() => { handleSortChange({ target: { value: 'priceLowHigh' } }); setShowSortModal(false); }}>Price (Low to High)</button>
                  </li>
                  <li>
                    <button className="w-full text-left py-2" onClick={() => { handleSortChange({ target: { value: 'priceHighLow' } }); setShowSortModal(false); }}>Price (High to Low)</button>
                  </li>
                </ul>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="relative mt-2 inline-flex items-center hidden md:flex space-x-2 mb-4">
              ...
              <div className={`${viewMode === 'grid' ? 'grid grid-cols-2 gap-4' : 'flex flex-col gap-4'} md:grid md:grid-cols-3 lg:grid-cols-5`}>
                {getFilteredSortedProducts().map((product) => {
                  ...
                  // List view card for mobile
                  if (viewMode === 'list') {
                    return (
                      <Link to={`/product/${product._id}`} key={product._id} className="w-full">
                        <div className="relative flex bg-white mt-2 border border-gray-200 rounded-lg shadow-md p-2 items-center">
                  ...
                  return (
                    <Link
                      ...
                      className="w-full"
                    >
                      <div className="bg-white border border-gray-200 mt-2 rounded-lg shadow-md p-4 sm:p-4 relative transform transition duration-300 hover:shadow-lg">
                        {/* Discount Badge */}
                        {defaultPackage && (
                        ...
                        <div className="absolute top-2 right-2 sm:right-4 flex flex-col items-center space-y-2">
                          <img
                            src={
