    ...
    async (term) => {
      ...
      if (term.trim() && eStoreId) {
        try {
          ...
          const data = await response.json();
          // Filter products that match the search term in either title or sub_title AND have the e-store super_cat_id
          const filteredResults = data.filter(
            (product) =>
              (product.title.toLowerCase().includes(term.toLowerCase()) ||
                (product.sub_title &&
                  product.sub_title.toLowerCase().includes(term.toLowerCase()))) &&
