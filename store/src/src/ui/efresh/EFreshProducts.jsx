...
const EfreshProducts = () => {
  ...
  // Fetch and filter products based on the e-store supercategory
  useEffect(() => {
    if (!eStoreId) return;
    ...
    const fetchProducts = async () => {
      try {
        ...
        // Filter products that belong to e-store category
  ...
  const handleShare = (product) => {
    const shareUrl = `${window.location.origin}/productpage/${product._id}`;
  ...
  const ProductDialog = ({ product, onClose, isOpen }) => {
    ...
    useEffect(() => {
      ...
      // Cleanup to reset overflow when dialog closes
      return () => {
        document.body.style.overflow = "auto";
      };
    }, [isOpen]);
    ...
    const handleAddToCart = (product, pkg) => {
      ...
      // Update local state for this specific package
    ...
    const handleRemoveFromCart = (product, pkg) => {
      ...
      if (currentQuantity > 0) {
        ...
        if (newQuantity === 0) {
          // Remove this specific package variant from cart
